...
void OnInit()
{
    ...
    SetIndexBuffer(2, prePlusVI, INDICATOR_CALCULATIONS);
    SetIndexBuffer(3, preMinusVI, INDICATOR_CALCULATIONS);
    SetIndexBuffer(4, TEMA_1_PlusVI, INDICATOR_CALCULATIONS);
    SetIndexBuffer(5, TEMA_1_MinusVI, INDICATOR_CALCULATIONS);
    SetIndexBuffer(6, TEMA_2_PlusVI, INDICATOR_CALCULATIONS);
    SetIndexBuffer(7, TEMA_2_MinusVI, INDICATOR_CALCULATIONS);
    SetIndexBuffer(8, TEMA_3_PlusVI, INDICATOR_CALCULATIONS);
    SetIndexBuffer(9, TEMA_3_MinusVI, INDICATOR_CALCULATIONS);
    SetIndexBuffer(10, UpperTFShift, INDICATOR_CALCULATIONS);
                ...
                const int &spread[])
{
    ...
    if (pos < IndPeriod - 1) // Pre-fill upper timeframe buffer.
    {
        ...
        if (Timeframe != Period())
        {
            for (int i = pos; i < IndPeriod - 1 && !IsStopped(); i++)
            {
                ...
                int shift = index;
                if (Timeframe != Period()) shift = iBarShift(_Symbol, Timeframe, iTime(_Symbol, PERIOD_CURRENT, index), false);
                if (shift == -1) shift = iBarShiftCustom(_Symbol, Timeframe, iTime(_Symbol, PERIOD_CURRENT, index)); // Standard iBarShift() failed (bug). Try custom, but slow.
                if (shift == -1)
                {
                    Print("iBarShift() and iBarShiftCustom() both failed.");
                    return 0;
                }
    ...
    for (int i = pos; i < rates_total && !IsStopped(); i++)
    {
        ...
        if (Timeframe != Period())
        {
            shift = iBarShift(_Symbol, Timeframe, iTime(_Symbol, PERIOD_CURRENT, index), false);
            if (shift == -1) shift = iBarShiftCustom(_Symbol, Timeframe, iTime(_Symbol, PERIOD_CURRENT, index)); // Standard iBarShift() failed (bug). Try custom, but slow.
            if (shift == -1)
            {
                Print("iBarShift() and iBarShiftCustom() both failed.");
...
void IssueAlerts(string message)
{
    ...
    message = message; // Native alerts in MT5 don't need indicator/symbol/period information.
        ...
        if (TypeSmoothing == SMOOTHING_SIMPLE)
        {
            ...
            if ((First) && (UpperTFShift[i] - UpperTFShift[0] >= IndPeriod + Smoothing)) // First smoothed element.
            {
                for (int j = 0, k = 0; k < Smoothing; k++, j++)
                {
                    ...
                    if (j >= i) return false; // Prevent array out of range errors.
